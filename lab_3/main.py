...
class Lab3:
    ...
    def create_tabl_random(self, n):
        ...
        for i in range(n):
            self.tabl.append([])
            for j in range(n):
                self.tabl[i].append(random.randint(0, 20))
        for i in range(n):
            for j in range(n):
                if i == j:
                    self.tabl[i][j] = 0
    ...
    def error(self):
        ...
        self.slave.focus_set()
        self.slave.minsize(300, 100)
        self.slave.maxsize(1000, 100)
        self.slave['bg'] = 'seashell'
        self.slave.wm_geometry("+600+250")
        Label(self.slave, text='Некоректно задано початкову та\або кінцеву вершину!',
              justify=LEFT, font="Arial 17 bold", bg='seashell').pack(fill='both')

    def main_window(self):
        self.root = Tk()
        self.root.minsize(780, 350)
        self.root.maxsize(1000, 350)
